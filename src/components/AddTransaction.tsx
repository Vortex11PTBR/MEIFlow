  ...
  async function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        setDescription(''); setAmount(''); setAICategory(null); setCategory('Outros')
      } catch (e) {
  ...
  return (
    <form onSubmit={submit} className="bg-white/[0.028] border border-white/[0.07] rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="font-mono text-[11px] text-slate-400 uppercase tracking-widest">Novo Lançamento</p>
        {aiLoading && (
        ...
        {aiCategory && !aiLoading && (
          <span className="font-mono text-[10px] text-purple-400">✦ IA: {aiCategory}</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div>
          <label className="font-mono text-[10px] text-slate-500 uppercase tracking-wider block mb-1">
            Categoria {aiCategory && <span className="text-purple-400 ml-1">✦ IA sugeriu</span>}
          </label>
          <select
            value={category}
