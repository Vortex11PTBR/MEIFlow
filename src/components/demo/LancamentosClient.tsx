...
const PERIOD_LABELS = { week: 'Esta semana', month: 'Este mês', year: 'Este ano' } as const

export function LancamentosClient({ transactions }: { transactions: SerializedTx[] }) {
        ...
        {filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((tx, i) => (
                  <tr key={tx.id} className={`border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${i % 2 === 0 ? '' : 'bg-slate-50/50 dark:bg-slate-800/50'}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border ${tx.type === 'INCOME' ? 'text-green-700 dark:text-green-300 border-green-200 dark:border-green-400/30 bg-green-50 dark:bg-green-400/10' : 'text-red-700 dark:text-red-300 border-red-200 dark:border-red-400/30 bg-red-50 dark:bg-red-400/10'}`}>
                        {tx.type === 'INCOME' ? 'Receita' : 'Despesa'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className={`font-mono text-sm font-semibold tabular-nums ${tx.type === 'INCOME' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
